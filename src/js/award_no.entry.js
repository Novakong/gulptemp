...
var kl = kp({
  list:[
    {src:'http://bos.bj.baidubce.com/we-gd/olay/2016cny/img/awardpage/bg_no.jpg'},
    {src:'http://bos.bj.baidubce.com/we-gd/olay/2016cny/img/awardpage/btn_detail.png'},
    {src:'http://bos.bj.baidubce.com/we-gd/olay/2016cny/img/awardpage/detail_no.png'}
  ],
  success:function(){
...
$(function(){
  ...
  $('.abtn').on('touchstart', function(e){
    ...
    if(tar.hasClass('a1')){
      _smq.push(['custom','back','BCfreeget']);
      ...
      getfacialcard();
      _smq.push(['custom','back','BC118']);
      gaClick('coupon-facial');
    }
    else if(tar.hasClass('a4')){
      getskintest();
      _smq.push(['custom','back','BCskintest']);
...
function getskintest(){
  ...
  $.ajax({
    type:'POST',
    url:'http://campaign.olay.com.cn/2016olaycny/act/?type=getSkinTestCard',
...
function getfacialcard(){
  ...
  $.ajax({
    type:'POST',
    url:'http://campaign.olay.com.cn/2016olaycny/act/?type=getFacialCard',
    success:function(data){
      var da = JSON.parse(data);
      $('.mask_ajax').addClass('hide');
      if(da.result == "0" || da.result == 0){
